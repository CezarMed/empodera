  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Agregar Copago a la cuenta del beneficiario</h1>
          </div>
          <div class="col-sm-6">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label>Tipo de copago</label>
                 <select class="custom-select form-control-border" id="tipocopago" name="tipocopago">
                  <option value="Acreditado">Acreditado</option>
                  <option value="Depositado">Depositado</option>
                  </select>
                 <input type="hidden" id="altaCopago" name="altaCopago" value="altaCopago">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                 ...
                 <input type="text" class="form-control" id="monto" name="monto" placeholder="Monto del copago" onkeyup="eliminarEspacio()" required>
                </div>
              </div>
            </div>
          </div> 
          <div class="card-footer">
            <button type="submit" class="btn btn-success">Enviar copago</button>
          </div>          
        </div>
      </form>
      </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <button type="button" class="btn btn-success" onclick="aprobarCopago()">Procedente</button>
...
<div id="datosCopago" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <!-- Contenido del modal -->
    <div class="modal-content">
      <div class="modal-header"><b>DATOS DEL COPAGO</b>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        <div class="col-md-12">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="modal-body" id="tablaResult2">
            ...
            <button type="button" class="btn btn-danger" onclick="cancelarCopago()">Cancelar Copago</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
  function rechazarCopago(){
    ...
    var idCopago=$('#idCopago').val();
        ...
        $.ajax({
            url: 'consultas/rechazarCopago',
            type: "POST",
            data: {"idCopago":idCopago,"observaciones":observaciones},
               ...
               $('.modal-backdrop').remove();
                 $(function() {
                  ...
                  $(document).ready(function() {
                    Toast.fire({
                      icon: 'success',
                      title: ' El copago se CANCELO correctamente, ya no estara disponible'
                    })
                  });
                  });
            }
...
<script type="text/javascript">
  function aprobarCopago(){
    var observaciones=$('#observaciones').val();
    var idCopago=$('#idCopago').val();
    var montoAutoriza=$('#montoAutoriza').val();
    
        $.ajax({
            url: 'consultas/aprobarCopago.php',
            type: "POST",
            data: {"idCopago":idCopago,"observaciones":observaciones, "montoAutoriza":montoAutoriza},
            success: function (cmb) {
               $("#datosCopagos").load(" #datosCopagos");
               $('#aprobarCopago').modal('hide');
               $('.modal-backdrop').remove();
                 $(function() {
                  ...
                  $(document).ready(function() {
                    Toast.fire({
                      icon: 'success',
                      title: ' El copago se AUTORIZO correctamente, en este momento ya se puede asignar a un Servicio del benficiario'
                    })
                  });
                  });
            }
...
<script type="text/javascript">
        ...
        // alert((this).data('id'));
        e.preventDefault();
        var idCopago = $(this).data('id'); 
        // alert(uid);
        $.ajax({
            url: 'consultas/consultasCopagosAsignados',
            type: "POST",
            data: {"idCopago":idCopago},
            success: function (cmb) {
                     // alert(cmb);
      ...
      $(document).on('click', '#btnView2', function(e){
        // alert((this).data('id'));
        e.preventDefault();
        var idCopago = $(this).data('id'); 
        // alert(uid);
...
<script type="text/javascript">
  function cancelarCopago(){
    var idCopago=$('#idCopago3').val();
    
        $.ajax({
            url: 'consultas/cancelarCopago.php',
            type: "POST",
            data: {"idCopago":idCopago},
            success: function (cmb) {
               // alert(cmb);
               ...
               $('.modal-backdrop').remove();
                 $(function() {
                  ...
                  $(document).ready(function() {
                    Toast.fire({
                      icon: 'success',
                      title: ' El copago se CANCELO correctamente, en este momento ya no esta disponible en la cuenta general del beneficiario'
                    })
                  });
                  });
            }
