...
<table>
    ...
    $query4="SELECT s.folioEmpodera,s.idServicio,s.servicio,s.monto,s.montoFacturable,b.fechaNacimiento, b.tipoPrograma, b.subgrupo, b.nombreBeneficiario, cie.cie10, cie.diagnosticocie10, cpt.cpt4, cpt.tratamientocpt4, scp.folioCierre, s.fechaAlta, te.descripcionEstatus, nm.fechaElaboracion, nm.motivoatencion  FROM servicios s 
    ...
    <tr>
        ...
        <td><?php echo $registros['tipoPrograma']; ?></td>
        <td><?php echo  str_replace("-","",$registros['fechaNacimiento']) ?><?php if ($i<=9) { echo "00000"; } else { echo "0000";} ?><?php echo $i ?></td>
    ...
    $totalmedicos=$extraidonotas['totalmedico'];
        ?>
        <td><?php echo ($totalgastosmedicos+$totalmedicos) ?></td>
        <?php 
    $querytotalmed2="SELECT sum(cirujano+anestesiologo+ayudante) as totalhonorarios from notaspreoperatorias np  
